...
8.  `<input>` 和 `<button>` 必须指明默认的 type，`<form>` 必须有默认的 method，其它属性可以留空，这样可避免在不同浏览器下产生行为上的差异。
9.  为增强语义化，按钮用 `<button>` 而不用 `<input>`。
10. 前台页面不用 table 做布局，也不要在本该用表格的地方用 `<ul>` 或其它标签代替，table 的目的是用来显示表格状的数据。
...
12. html功能块之间写明注释，以便于此功能块的功能说明或嵌套提示，注释亦精不亦多。如：
...
14. 把 css 调用写在 head 头部，不需预先执行的 JS 尽量写在页面尾部，**不要在 html 代码中间插入 script 代码块**，script 代码块应与 html 之间留一个空行，script 代码块开头无需缩进：
...
15. 不在 html 中混合 JS 及 event 事件。
16. 明确指定图片的 width 和 height。不仅对 seo 有用，对因各种原因无法显示图片的情况下，也能保持布局样式基本不变。
17. 通过给元素设置自定义属性来存放与 JS 交互的数据，属性名格式为 data-xx(-xx)，中间用中划线连接，例如：data-lazyload-url。
18. 禁止单独用 `<div>` 标签做容器，使用 `<div>` 时必须至少带有一个类名。更不要它代替 `<p>` 标签，因为 `<div>` 标签没有明确的含义，应该根据各标签的语义，做到该用什么标签就用什么标签。
