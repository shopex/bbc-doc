...
|  tag_mode |  |  VARCHAR(6) | Yes | normal | No |
...
|  status | 支付状态 |  VARCHAR(8) | Yes | ready | No |
|  pay_type | 支付类型 |  VARCHAR(6) | Yes | online | No |
...
|  pay_type |  |  VARCHAR(7) | Yes | offline | No |
|  status |  |  VARCHAR(8) | Yes | ready | No |
...
|  platform | 模板终端 |  VARCHAR(3) | Yes | pc | No |
...
|  aftersales_type |  |  VARCHAR(16) | Yes | ONLY_REFUND | No |
|  progress |  |  VARCHAR(1) | Yes | 0 | No |
|  status |  |  VARCHAR(1) | Yes | 0 | No |
...
|  aftersales_bn |  |  BIGINT UNSIGNED | Yes |  | No |
|  refunds_type |  |  VARCHAR(1) | Yes | 0 | No |
|  status |  |  VARCHAR(1) | Yes | 0 | No |
...
|  show_type |  |  VARCHAR(5) | Yes | text | No |
|  prop_type |  |  VARCHAR(6) | Yes | spec | No |
...
|  settlement_status |  |  VARCHAR(1) | No | 1 | No |
...
|  pv_type |  |  VARCHAR(6) | Yes | select | No |
...
|  item_type | 商品类型 |  VARCHAR(7) | Yes | item | No |
...
|  valuation | 运费计算参数来源 |  VARCHAR(1) | No | 1 | No |
...
|  status | 是否开启 |  VARCHAR(3) | No | on | No |
...
|  used_platform | 使用平台 |  VARCHAR(1) | Yes | 0 | No |
...
|  use_bound | 使用范围 |  VARCHAR(1) | No | 0 | No |
...
|  status |  |  VARCHAR(7) | No | WAIT | No |
|  appeal_type |  |  VARCHAR(12) | No | APPLY_UPDATE | No |
...
|  shop_id | 店铺id |  INT UNSIGNED | No |  | No |
|  seller_type |  |  VARCHAR(1) | Yes | 0 | No |
...
|  status | 提交状态 |  VARCHAR(6) | No | active | No |
...
|  shop_type | 店铺类型id |  VARCHAR(5) | Yes |  | No |
...
|  state | 当前任务是否已经执行过 |  VARCHAR(8) | No | ready | No |
...
|  op_role | 操作角色 |  VARCHAR(9) | Yes | system | No |
...
|  modified_time | 最后更新时间 |  INT UNSIGNED | No |  | No |
|  status | 子订单状态 |  VARCHAR(24) | Yes | WAIT_BUYER_PAY | No |
|  aftersales_status | 售后状态 |  VARCHAR(25) | No |  | No |
...
|  status | 订单状态 |  VARCHAR(24) | Yes |  | No |
|  cancel_reason |  |  LONGTEXT | No |  | No |
|  pay_type | 支付类型 |  VARCHAR(7) | Yes | online | No |
...
|  trade_from | 订单来源 |  VARCHAR(6) | No | pc | No |
...
|  step_trade_status | 分阶段付款的订单状态 |  VARCHAR(1) | No |  | No |
|  step_paid_fee | 分阶段付款的已付金额 |  NUMERIC(20, 3) | No |  | No |
|  shipping_type | 创建交易时的物流方式 |  VARCHAR(7) | No |  | No |
...
|  tid |  |  BIGINT UNSIGNED | Yes |  | No |
|  reason |  |  VARCHAR(255) | No |  | No |
|  status |  |  VARCHAR(15) | Yes | WAIT_PROCESSING | No |
...
|  behavior_type | 行为类型 |  VARCHAR(7) | No | obtain | No |
